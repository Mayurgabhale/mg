...
we want only this :: 
  ...
  try {
    ...
    // ---------- SHEET 1: APAC Employee ----------
    const wsDetails = wb.addWorksheet('APAC Employee');

    // Headers
    const detailsHeaders = [
      'Sr.No', 'Date', 'Time',
      'Employee ID', 'Card Number', 'Name',
      'Personnel Type', 'Company', 'Primary Location',
      'Door', 'Partition'
    ];
    ...
    detailsTitle.value = `APAC Details — ${format(pickedDate, 'EEEE, d MMMM, yyyy')}`;
    ...
    (detailRows || []).forEach((r, i) => {
      const dateVal = (r.LocaleMessageTime?.slice(0, 10)) || '';
      ...
      const location = r.PrimaryLocation || '';
      const door = r.Door || '';
      const partition = formatPartition(r.PartitionNameFriendly) || '';

      const row = wsDetails.addRow([
        i + 1, dateVal, timeVal, empId, card, name,
        ptype, company, location, door, partition
      ]);
    ...
    // ---------- SHEET 2: APAC Summary ----------
    const ws = wb.addWorksheet('APAC Summary');
...
++ i want above export same for below, as it is..
  also this this  wsDetails.columns.forEach((col, idx) => {
        let maxLen = 0;
        col.eachCell({ includeEmpty: true }, c => {
          const v = c.value === null || c.value === undefined ? '' : String(c.value).trim();
          if (v.length > maxLen) maxLen = v.length;
...
----- and this Sr.No	Date	Time	Employee Name	Employee ID	Personal Type	Door Name	Location
ok
