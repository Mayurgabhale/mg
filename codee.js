...
IN THIS TODAY DATA NOT DIPLSY, BUT PAST CODE ALOS DISPLY, WHY CHEKC 

COMMNETED CODE AARE PAST CODE OK READ THIS CAREFULLYM 
  ...
  const [indexByDate, setIndexByDate] = useState({
    ...
    const buildIndexAndSet = (json) => {
      ...
      (json.details || []).forEach(r => {
        ...
        // fall back to cached data if fetch 
    ...
    const filtered = rows.filter(r => {
      if (backendFilterKey) {
        const ok = r.Partitio
      const excelModule = await import('excely ||  '';
