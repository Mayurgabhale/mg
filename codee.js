...
// helper: fallback regex to extract floor number from Door string
function extractFloorFromDoor(door) {
  if (!door) return "Unknown";
  const match = door.match(/HQ\.\s*(\d{2})\./i);
  if (match) return `Floor ${parseInt(match[1], 10)}`;
  return "Unknown";
}
...
async function getRejections(req, res) {
  let pool;
  try {
    // ✅ Always get the pool via getPool()
    pool = await denver.getPool();
    if (!pool) {
      console.warn("⚠️ Denver pool not available");
      return res.status(503).json({ error: "Denver DB unavailable" });
    }
  } catch (err) {
    console.error("❌ Failed to get Denver pool:", err);
    return res.status(503).json({ error: "Denver DB unavailable" });
  }
  ...
  try {
    ...
    // Post-process rows for floor mapping
    ...
    // Reset the pool if something went wrong
    try {
      if (pool && typeof pool.close === "function") await pool.close();
      denverPoolPromise = null; // will be re-created on next getPool()
    } catch (e) {}
