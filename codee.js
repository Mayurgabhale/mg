...
exports.fetchHistoricalData = async ({ location = null }) => {
  ...
  // 2️⃣ Build location filter (note: starts with AND so it can be appended after WHERE rn = 1)
  const filterClause = location
    ? `AND PartitionNameFriendly = @location`
    : `AND PartitionNameFriendly IN (${quoteList([
  ...
  // Debug log (first N chars only) — remove or reduce in production
  try {
    console.log('--- Historical query (head) ---');
    console.log(query.slice(0, 2000));
    console.log('--- end query head ---');
    ...
    // Log error and the full-ish query for troubleshooting
    console.error('fetchHistoricalData SQL error:', err.message || err);
    // Optionally log full query if needed (careful in prod)
    // console.error('Full query:', query);
    throw err; // rethrow so controller can handle and return appropriate 500
  }
};