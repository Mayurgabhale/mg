VM235 react-dom.development.js:29905 Download the React DevTools for a better development experience: https://reactjs.org/link/react-devtools
analysis_table.html:148 Error executing header script SyntaxError: unknown: Unexpected token (19:200) (at VM234 babel.min.js:7:10099)
  17 |     useEffect(() => {
  18 |       if (window.flatpickr && fromRef.current && toRef.current) {
> 19 |         fromFp.current = window.flatpickr(fromRef.current, { dateFormat:"Y-m-d", defaultDate:dateFrom, allowInput:true, onChange:sd => { if(sd.length){ setDateFrom(formatDateISO(sd[0])); toFp.current?.set('minDate', formatDateISO(sd[0])); } } });
     |                                                                                                                                                                                                         ^
  20 |         toFp.current = window.flatpickr(toRef.current, { dateFormat:"Y-m-d", defaultDate:dateTo, allowInput:true, onChange:sd => { if(sd.length){ setDateTo(formatDateISO(sd[0])); fromFp.current?.set('maxDate', formatDateISO(sd[0])); } } });
  21 |       }
  22 |       return ()=>{ fromFp.current?.destroy(); toFp.current?.destroy(); }
    at J.raise (VM234 babel.min.js:7:10099)
    at X.unexpected (VM234 babel.min.js:5:27476)
    at te.parseExprAtom (VM234 babel.min.js:6:30787)
    at t.parseExprAtom (VM234 babel.min.js:8:22540)
    at te.parseExprSubscripts (VM234 babel.min.js:6:25684)
    at te.parseMaybeUnary (VM234 babel.min.js:6:25267)
    at te.parseExprOps (VM234 babel.min.js:6:23839)
    at te.parseMaybeConditional (VM234 babel.min.js:6:23430)
    at te.parseMaybeAssign (VM234 babel.min.js:6:22650)
    at t.parseMaybeAssign (VM234 babel.min.js:8:12561)
(anonymous) @ analysis_table.html:148
insertHeaderAndRunScripts @ analysis_table.html:136
(anonymous) @ analysis_table.html:154
Promise.then
(anonymous) @ analysis_table.html:153
(anonymous) @ analysis_table.html:470
...
/*
  analysis_table.html
  - loads header.html into #header-root and executes its scripts
